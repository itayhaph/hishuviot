...
def run_match(agent, goal_keeper, kicks_per_match=50):
    # Executes a single match of 50 penalty kicks, and returns the total reward accumulated by the agent
    total_reward = 0
...
def tune_agent(agent_class, param_grid, matches_count=1000):
    # Generalized function to perform grid search over hyperparameters (could be 1 or 2 parameters)
    ...
    if len(param_names) == 2:
        ...
        # MeanQLearner/PreferenceLearner
...
# Defining initial search space for all three agents
agents_to_test = [
    (MeanQLearner, {'epsilon': [0.01, 0.1, 0.2, 0.4]}),
    (FixedRateQLearner, {'eta': [0.01, 0.1, 0.5], 'epsilon': [0.01, 0.1, 0.2]}),
    (PreferenceLearner, {'eta': [0.01, 0.1, 0.5, 0.8, 1.2]})
]
...
for idx, (agent_class, initial_grid) in enumerate(agents_to_test):
    ...
    print(f"Tuning for {agent_name}")
    
    # First we do fast preliminary search (300 matches)
    pre_results, p_names = tune_agent(agent_class, initial_grid, matches_count=300)
    
    # Then define search range around best preliminary values
    refined_grid = {}
    ...
    # At last we do a full search - 1000 matches per combination
    ...
    # Section 3.e, Generate unbiased estimate with fresh environment instances
